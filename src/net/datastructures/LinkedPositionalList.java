...
public class LinkedPositionalList<E> implements PositionalList<E> {
	...
	/**
	 * Retorna un mapa donde la clave es el elemento y el valor es una lista
	 * posicional de todas los posiciones donde se ubica ese elemento
	 * 
	 */
	public Map<E, PositionalList<Position<E>>> mapEquals() {
		Map<E, PositionalList<Position<E>>> map = new ChainHashMap<E, PositionalList<Position<E>>>();

		PositionalList<Position<E>> lp;
		for (Position<E> p = first(); p != null; p = after(p)) {
			lp = map.get(p.getElement());
			if (lp == null) {
				lp = new LinkedPositionalList<Position<E>>();
				map.put(p.getElement(), lp);
			}
			lp.addLast(p);
		}
		return map;
